...
## 2. Integración de IA (Gemini 2.5 Flash / 2.0 Flash - Free Tier)
- **Endpoint:** Configura un API Route en `/app/api/chat/route.ts`.
- **Modelo:** Forzar el uso de `gemini-2.5-flash` como modelo principal, implementando un mecanismo de *Fallback* automático hacia `gemini-2.0-flash` para evadir errores 503 (servidor saturado) y 429 (límite de peticiones), sumando así las cuotas gratuitas de ambos modelos.
